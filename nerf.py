...
def test(hn, hf, dataset, chunk_size=10, img_index=0, nb_bins=192, H=400, W=400):
    ...
    data = []   # Lista de valores de los pixeles regenerados
    for i in range(int(np.ceil(H / chunk_size))):   # Iteramos sobre los chunks
        # Obtenemos chunks de rayos
...
def render_rays(nerf_model, ray_origins, ray_directions, hn=0, hf=0.5, nb_bins=192):
    ...
    # Calculamos los puntos 3D de cada rayo
    ...
    # Calculamos los valores de los pixeles como la suma de colores en cada rayo
    c = (weights * colors).sum(dim=1)
    weight_sum = weights.sum(-1).sum(-1)  # Regularizamos para fondo blanco
